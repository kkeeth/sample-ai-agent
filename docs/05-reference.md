...
> **これは通読するものではありません。** 当日は [ハンズオン](04-handson.md) の45分だけやれば十分です。
> ここは、そのあと**詰まったときに該当ステップだけ引く**ためのリファレンスです。
>
> | こんなとき | 見るところ |
> | --- | --- |
...
> **当日の進行指示は [ハンズオン](04-handson.md) です。**参加者にこの手順書を通しでやらせないでください。
> ここに載せているのは、運営が「どのチームがどこまで来ているか」を判定するための目安です。

参加者は Step 5〜8 の安全装置が**入った状態**から始めます（テンプレートに実装済み）。到達目標は「自分のツールでそこまで到達したか」ではなく「動くか」です。

| 合宿の時刻                    | 見るところ                                  | 判定                                       |
| ----------------------------- | ------------------------------------------- | ------------------------------------------ |
| 初日 10:45（ハンズオン終了）  | [ハンズオン](04-handson.md) の3まで          | `description` を直すと呼ばれ方が変わった   |
| 初日 13:00                    | 自分のツールが1個                            | ループが1周でも回る                         |
| 初日 16:00（中間チェック）    | 自分のツールが2〜3個（Step 3〜4 相当）       | 質問を変えると呼ばれるツールが変わる       |
| 2日目 12:00                   | 書き込み系があるなら `WRITE_TOOLS`（Step 8） | 承認ゲートを通っている                     |
| 2日目 15:00（コードフリーズ） | デモ                                        | 通ればよい                                 |

**中間チェックで見るのは「動くものが1つあるか」だけ**です。設計の良し悪しには触れません。触れると手戻りが発生します。

---

_関連： [これだけ](00-tldr.md) ／ [運営ランブック](01-runbook.md) ／ [電話越しの現場監督](02-agent-primer.md) ／ [キックオフ台本](03-kickoff-script.md) ／ [ハンズオン](04-handson.md)_
