...
> **改良版の比喩**
> **「電話越しの、記憶をなくす現場監督」**。現場を見たことがなく、作業員に直接触れず、毎回これまでの報告書の束を読み直してから、次の一手だけを指示してくる監督。
> この奇妙さの一つひとつが、技術的な事実に対応しています。
...
flowchart LR
    ...
    Loop -->|"① 依頼 ＋ 手の一覧 ＋ これまでの全履歴"| Model
    Model -->|"② read_file を path=… で呼べ"| Loop
    Loop -->|"③ 関数を実行"| Tools
    Tools -->|"④ 戻り値"| Loop
...
ここまでの図をそのままコードにしたものです。テンプレートの `src/agent.ts` に入っています。

```ts
// ① 名簿：モデルに「こういう手が使える」と教える。実装ではなく説明だけ
const tools = [{
  name: "read_file",
...
| どんな「手」が要るか決める | 1時間 | ツールを3〜5個に絞る。設計の本体 |
